class SportsJson {
  static Map<String, dynamic> get sportsSection => {
    "pageHeading": "Sports Infrastructure - Playground",
    "section": "sportsSector",
    "apiPaths": {"post": "v1/candidate/education"},
    "fields": [
      {
        "name": "state",
        "label": "1. State",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "State is required.",
      },
      {
        "name": "district",
        "label": "2. District",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "District is required.",
      },
      {
        "name": "blockTown",
        "label": "3. Block/Town",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "Block/Town is required.",
      },
      {
        "name": "pmjvkProjectId",
        "label": "4. PMJVK Project ID",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "PMJVK Project ID is required.",
      },
      {
        "name": "projectName",
        "label": "5. Project Name",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "Project Name is required.",
      },
      {
        "name": "sector",
        "label": "6. Sector",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "Sector is required.",
      },
      {
        "name": "subSectorProjectType",
        "label": "7. Sub-sector/Project Type",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "Sub-sector/Project Type is required.",
      },
      {
        "name": "responsibleDepartment",
        "label": "8. Responsible Department",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "Responsible Department is required.",
      },
      {
        "name": "dateOfCompletion",
        "label": "9. Date of Completion",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "Date of Completion is required.",
      },
      {
        "name": "dateOfFunctionalityCheck",
        "label": "10. Date of Functionality Check/Assessment",
        "type": "date",
        "required": true,
        "validationMsg": "Please select the date of functionality check.",
      },
      {
        "name": "officerConductingCheck",
        "label": "11. Officer Conducting Check/Assessment",
        "type": "text",
        "required": true,
        "validationMsg": "Please enter the officer's name.",
      },
      {
        "name": "mobileNumber",
        "label": "12. Mobile Number",
        "type": "text",
        "required": true,
        "maxLength": 10,
        "keyboardType": "number",
        "validationMsg": "Please enter a valid mobile number.",
      },
      {
        "name": "geoCoordinates",
        "label": "13. Geo Lat-Long Coordinates",
        "type": "text",
        "required": true,
        "disable": true,
        "validationMsg": "Location is required.",
      },

      {
        "name": "inspectionRemarks",
        "label": "14. Inspection Remarks",
        "type": "textarea",
        "required": false,
        "maxLength": 500,
      },
      {
        "name": "openingDate",
        "label": "15. Date of Opening/Inaugration",
        "type": "date",
        "required": true,
        "validationMsg": "Please select the date of functionality check.",
      },
      {
        "name": "assetOptional",
        "label": "16. Is the asset operational?",
        "type": "radio",
        "required": true,
        "options": [
          {"label": "Yes", "value": true},
          {"label": "No", "value": false},
        ],
        "validationMsg": "Please select whether dropout has reduced.",
      },
      {
        "name": "nonOperationalRemarks",
        "label": "16.1. If non-operational, reason thereof",
        "type": "text",
        "required": true,
        "validationMsg": "Please enter current student enrolment.",
        "visibleWhen": [
          {"key": "assetOptional", "value": true},
        ],
      },
      {
        "name": "dailyUsers",
        "label": "17. Daily users",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of daily users.",
      },
      {
        "name": "schoolsUsingFacility",
        "label": "18. Schools using facility",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of schools using the facility.",
      },
      {
        "name": "sportsEventsPerYear",
        "label": "19. Sports events/year",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of sports events per year.",
      },
      {
        "name": "coachAvailable",
        "label": "20. Coach available",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of coaches available.",
      },
      {
        "name": "equipmentAvailable",
        "label": "21. Equipment available",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of equipment available.",
      },
      {
        "name": "lightingAvailable",
        "label": "22. Lighting available",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter lighting availability.",
      },
      {
        "name": "maintenanceLastDone",
        "label": "23. Maintenance (Last Done)",
        "type": "date",
        "required": true,
        "validationMsg": "Please select the last maintenance date.",
      },

      {
        "name": "isIndoorOrComplex",
        "label": "Is this Indoor Stadium / Sports Complex?",
        "type": "radio",
        "required": true,
        "options": [
          {"label": "Yes", "value": true},
          {"label": "No", "value": false},
        ],
        "validationMsg": "Please confirm",
      },

      // Indoor Stadium / Sports Complex
      {
        "name": "seatingCapacity",
        "label": "24. Seating capacity",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter seating capacity.",
      },
      {
        "name": "sportsDisciplinesConducted",
        "label": "25. Sports disciplines conducted",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of sports disciplines conducted.",
      },
      {
        "name": "registeredDailyUsers",
        "label": "26. Registered daily users",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of registered daily users.",
      },
      {
        "name": "competitionsOrganised",
        "label": "27. Competitions organised",
        "type": "text",
        "required": true,
        "keyboardType": "number",
        "validationMsg": "Please enter number of competitions organised.",
      },
      {
        "name": "indoorEquipmentAvailable",
        "label": "28. Equipment available",
        "type": "radio",
        "required": true,
        "options": [
          {"label": "Yes", "value": true},
          {"label": "No", "value": false},
        ],
        "validationMsg": "Please select whether equipment is available.",
      },
      {
        "name": "washroomsFunctional",
        "label": "29. Washrooms functional",
        "type": "radio",
        "required": true,
        "options": [
          {"label": "Yes", "value": true},
          {"label": "No", "value": false},
        ],
        "validationMsg": "Please select whether washrooms are functional.",
      },
    ],
  };
}
